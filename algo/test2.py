...
class Processor:
    def process_data(self, subjects: List[Subject], teachers: List[Teacher]) -> List[Tuple[Subject, Teacher]]:
        ...
        for subject in subjects:
            ...
            # Если есть подходящие учителя, выбираем того с наименьшей загруженностью
            if eligible_teachers:
                selected_teacher = min(eligible_teachers,
                                       key=lambda t: t.loaded_hours)  # Выбираем учителя с наименьшей загруженностью
    ...
    def calculate_load(self, teachers: List[Teacher]) -> Dict[int, int]:
        load_summary = {teacher.teacher_id: teacher.loaded_hours for teacher in teachers}
        return load_summary


# Пример использования с большим количеством тестовых данных
if __name__ == "__main__":
    ...
    # Подсчет загруженности преподавателей
    load_summary = processor.calculate_load(teachers)

    print("\nЗагруженность преподавателей:")
    for teacher_id, load in load_summary.items():
        print(f"Преподаватель {teacher_id}: {load} часов")